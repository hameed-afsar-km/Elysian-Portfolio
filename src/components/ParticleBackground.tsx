...
export default function ParticleBackground() {
  ...
  useEffect(() => {
    ...
    // Floating large outline shapes (very clean, thin lines)
    class FloatingShape {
      x: number;
      y: number;
      size: number;
    ...
    function animate() {
      ...
      // Draw subtle grid points and crosses
      const spacing = 80;
      ctx.lineWidth = 1;
      ...
      for (let x = spacing / 2; x < width; x += spacing) {
        for (let y = spacing / 2; y < height; y += spacing) {
          ...
          // Mouse proximity reaction: zoom and scale up the cross
          if (dist < 180) {
          ...
          // Occasionally render small index coordinates
